...
Shader "Google/Maps/Shaders/Worldspace Buildings" {
  Properties {
    ...
    _ScaleXZ("Texture Scale (X and Z)", Float) = 500.0
    _ScaleY("Texture Scale (Y)", Float) = 1000.0
  ...
  SubShader {
    Pass {
      ...
      uniform float _Shininess, _ScaleXZ, _ScaleY;
      ...
      vertexOutput vert(vertexInput v) {
        ...
        output.uv = float2(
            (output.vertex.x + _Offset.x) / _ScaleXZ,
            (output.vertex.z + _Offset.z) / _ScaleXZ
            + output.vertex.y / _ScaleY);
