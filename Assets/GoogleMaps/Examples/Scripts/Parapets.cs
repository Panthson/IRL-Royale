...
public sealed class Parapets : MonoBehaviour {
  ...
  private void Start () {
    ...
    // If a building is modified by the loading of extra tiles, update its associated parapet.
    dynamicMapsService.MapsService.Events.ExtrudedStructureEvents.DidModify.AddListener(args => {
      Material roofMaterial = args.GameObject.GetComponent<MeshRenderer>().sharedMaterials[1];
      Extruder.UpdateBuildingParapet(args.GameObject, args.MapFeature.Shape, roofMaterial);
    });
