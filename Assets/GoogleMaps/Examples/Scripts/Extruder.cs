...
public static class Extruder {
  ...
  public static GameObject[] AddAreaOutline(GameObject areaObject,
    ...
    // Build an extrusion in local space (at origin). Note that GenerateBoundaryEdges currently
    // incorrectly handles some pathological cases, for example, area chunks with a single edge
    // starting and ending outside the enclosing tile, so some very occasional misaligned
    // extrusions are to be expected.
    List<Area.EdgeSequence> loops = PadEdgeSequences(area.GenerateBoundaryEdges());
